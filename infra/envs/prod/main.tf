/**
 * Prod stack — env-scoped resources within inference-expt.
 * Deploys to app.quantum-23.com (and api.quantum-23.com in Phase 1).
 */
...
locals {
  agent_orch_member = "serviceAccount:${module.iam_prod.agent_orchestrator_sa_email}"
}
...
module "cloud_sql" {
  ...
  iam_user_email    = module.iam_prod.agent_orchestrator_sa_email
  labels            = local.common_labels
...
module "firestore" {
  ...
  project_id = var.project_id
  env        = local.env
  location   = "nam5" # multi-region for prod
}
...
module "gke" {
  ...
  # Prod: warm 2 GPU nodes for HA + headroom.
  cpu_min_nodes = 2
  cpu_max_nodes = 5
  gpu_min_nodes = 2
  gpu_max_nodes = 4
...
module "weather" {
  ...
  min_instances    = 1 # warm pool for prod
  max_instances    = 15
  cpu              = "1"
  memory           = "1Gi"
  vpc_connector_id = data.terraform_remote_state.shared.outputs.vpc_connector_id
...
module "agent" {
  ...
  image                 = var.hello_image
  service_account_email = module.iam_prod.agent_orchestrator_sa_email

  min_instances    = 2 # warm pool for prod
  max_instances    = 20
  cpu              = "2"
  memory           = "2Gi"
  timeout          = "300s"
  ...
  env_vars = {
    ...
    CORS_ALLOW_ORIGINS  = "https://app.${var.domain}"
  ...
  dns_subdomain = "api" # api.quantum-23.com
  domain        = var.domain
...
module "frontend" {
  ...
  env_vars = {
    ...
    NEXT_PUBLIC_AGENT_URL = "https://api.${var.domain}"
  }

  ingress      = "INGRESS_TRAFFIC_ALL"
  allow_public = true

  dns_subdomain = "app" # app.quantum-23.com
  domain        = var.domain
  dns_zone_name = data.terraform_remote_state.shared.outputs.dns_zone_name
